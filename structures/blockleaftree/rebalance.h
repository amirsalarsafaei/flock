...
template <typename Tree> struct Rebalance {
  Tree *tree;
  ...
  void retire_node(node *x) { (&(tree->node_pool))->retire(x); }
  Rebalance(Tree *tree) : tree(tree) {}

  size_t priority(node *v) { return parlay::hash64_2(v->key); }
  ...
  void check_balance(node *p, node *l, node *r) {
    if (!l->is_leaf && priority(l) > priority(p))
      std::cout << "bad left priority: " << priority(l) << ", " << priority(p)
                << std::endl;
    if (!r->is_leaf && priority(r) > priority(p))
      std::cout << "bad right priority: " << priority(r) << ", " << priority(p)
                << std::endl;
  }
};
